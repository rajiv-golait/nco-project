...
def test_search_english(client):
    ...
    assert all(key in result for key in ["nco_code", "title", "description", "score", "confidence", "matched_synonyms"])
...
def test_get_occupation(client):
    ...
    nco_code = search_response.json()["results"][0]["nco_code"]
    
    # Get occupation details
    response = client.get(f"/occupation/{nco_code}")
    assert response.status_code == 200
    data = response.json()
    
    assert data["nco_code"] == nco_code
    assert all(key in data for key in ["title", "description", "synonyms", "examples"])


def test_get_occupation_not_found(client):
...
def test_search_low_confidence(client):
